...
(async()=>{
 ...
 const issues=[];const out=path.resolve(__dirname,'../projects/telegram-intel/assets');
 for(const [label,width,height] of [['1440',1440,900],['768',768,1024],['390',390,844]]){
  ...
  await page.goto('http://127.0.0.1:4315/site/case-telegram-intel.html',{waitUntil:'networkidle'});
  await page.locator('.intel-case img').evaluateAll(async images=>{await Promise.all(images.map(async image=>{image.loading='eager';try{await image.decode()}catch{}}))});
  const result=await page.evaluate(()=>({h1:document.querySelectorAll('h1').length,width:innerWidth,scrollWidth:document.documentElement.scrollWidth,broken:[...document.images].filter(i=>!i.complete||!i.naturalWidth).map(i=>i.src),videos:document.querySelectorAll('.intel-videos video').length,states:document.querySelectorAll('.intel-screen-grid img').length}));
