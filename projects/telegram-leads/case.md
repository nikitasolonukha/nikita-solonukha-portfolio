...
Основная портфолио-страница: `site/case-telegram-leads.html`. Она сохраняет настоящий обезличенный Telegram-кадр главным визуальным доказательством; desktop/mobile walkthrough ниже явно подписаны как безопасный локальный replay. Для мобильной подачи сделан кроп того же кадра, без изменения Telegram UI.
