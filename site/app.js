  ...
  $('#selected-list').innerHTML = selected.map((project, index) => `
    <article class="featured">
      ...
      <div class="featured-copy"><div><span class="featured-index">${String(index + 1).padStart(2, '0')} / ${String(selected.length).padStart(2, '0')}</span><span>${escapeHtml(project.category)}</span></div>
        <h3><a href="${projectUrl(project.id)}">${escapeHtml(project.name)} <span aria-hidden="true">${arrowIcon}</span></a></h3>
...
const arrowIcon='<svg class="inline-arrow" aria-hidden="true" viewBox="0 0 24 24" fill="none"><path d="M5 19 19 5M8 5h11v11" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';
const backIcon='<svg class="inline-arrow" aria-hidden="true" viewBox="0 0 24 24" fill="none"><path d="M19 12H5M11 6l-6 6 6 6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';
function projectFeature(id){if(id!=='protective-structures')return '';return `<section class="case-feature case-feature-kontur"><div class="shell"><p class="eyebrow">СИСТЕМА / САЙТ</p><div class="kontur-intro"><h2>Сложная система.<br><em>Ясное объяснение.</em></h2><p>Сайт раскрывает устройство защитной конструкции, затем показывает сферы применения и приводит к запросу расчёта.</p></div><figure class="kontur-main"><picture><source media="(max-width:760px)" srcset="${asset('../projects/protective-structures/assets/how-clean-390.png?v=settled-20260925-b')}"><img src="${asset('../projects/protective-structures/assets/how-clean-1440.png?v=settled-20260925-b')}" alt="Полностью загруженный раздел KONTUR: принцип защитной системы и четыре элемента конструкции" loading="lazy"></picture><figcaption>01 / Состав системы · экран сайта</figcaption></figure><div class="kontur-detail"><div><span>01</span><strong>Конструкция</strong><p>Три элемента объясняются на одном экране: опоры, каркас, сетчатое полотно.</p></div><div><span>02</span><strong>Применение</strong><p>От устройства системы — к энергетике, промышленности и другим объектам.</p></div><div><span>03</span><strong>Действие</strong><p>Посетитель может открыть контактную форму и запросить обсуждение объекта.</p></div></div><figure class="kontur-secondary"><picture><source media="(max-width:760px)" srcset="${asset('../projects/protective-structures/assets/industries-clean-390.png?v=settled-20260925-b')}"><img src="${asset('../projects/protective-structures/assets/industries-clean-1440.png?v=settled-20260925-b')}" alt="Полностью загруженный раздел KONTUR: отрасли применения защитной системы" loading="lazy"></picture><figcaption>02 / Области применения · экран сайта</figcaption></figure><p class="kontur-caveat">Иллюстрации показывают возможные сценарии применения конструкции.</p></div></section>`;}
...
function casePage(projects) {
  ...
  if (!item) {
    root.innerHTML = `<div class="case-error shell"><h1>Проект не найден.</h1><a href="work.html">Все работы ${arrowIcon}</a></div>`;
  ...
  const mobileMedia = media.find(entry => entry.mobile);
  const mainVideo = !presentationIds.has(id) && media.length ? `<section class="case-video-section"><div class="shell"><p class="eyebrow">Продукт в действии</p><div class="case-primary-desktop${mobileMedia && mobileMedia !== media[0] ? ' has-mobile' : ''}">${videoFigure(media[0], 0, item.name)}</div>${mobileMedia && mobileMedia !== media[0] ? `<div class="case-primary-mobile">${videoFigure(mobileMedia, 0, item.name)}</div>` : ''}</div></section>` : '';
  const secondary = !presentationIds.has(id) && media.length > 1 ? `<section class="case-secondary shell"><p class="eyebrow">Другие сценарии / mobile</p><div class="case-media-secondary">${media.slice(1, 5).map((entry, index) => `<div class="${entry === mobileMedia ? 'case-mobile-duplicate' : ''}">${videoFigure(entry, index + 1, item.name)}</div>`).join('')}</div></section>` : '';
  ...
  root.innerHTML = `<div class="case-title shell"><a class="back-link" href="work.html">${backIcon} Работы</a><p class="eyebrow">${escapeHtml(status)}</p><h1>${escapeHtml(caseTitles[id] || item.name)}</h1><p class="case-deck">${escapeHtml(item.shortDescription || item.subtitle)}</p><div class="case-title-meta"><div><span>Роль / услуги</span><strong>${escapeHtml(notes.role || 'Разработка')}</strong></div><div><span>Проект</span><strong>${escapeHtml(item.category)}</strong></div><div><span>Технологии</span><strong>${escapeHtml(item.stack === 'Подробности в исследовании' ? 'Указаны в материалах кейса' : item.stack)}</strong></div></div></div>
    ...
    <section class="case-story case-summary shell"><span class="eyebrow">${escapeHtml(item.name)} / о проекте</span><div><h2>${escapeHtml(item.caseIntro || item.intro)}</h2>${id === 'ppbot' ? '<div class="ppbot-impact" aria-label="Результаты проекта"><div><strong>176</strong><span>пользователей</span></div><div><strong>62</strong><span>оплативших</span></div><div><strong>≈35%</strong><span>конверсия в оплату</span></div></div>' : ''}<div class="case-story-facts case-summary-rows"><div><h3>Задача</h3><p>${escapeHtml(item.problem)}</p></div><div><h3>Что я сделал</h3><p>${escapeHtml(item.built)}</p></div><div><h3>Результат</h3><p>${escapeHtml(item.result)}</p></div><div><h3>Польза</h3><p>${escapeHtml(item.businessValue || '')}</p></div></div></div></section>
    ${mainVideo}${feature}${gallery}${secondary}
    <div class="case-next shell"><a href="${projectUrl(next.id)}"><span><small>Следующий кейс</small><br>${escapeHtml(next.name)}</span>${arrowIcon}</a><a class="case-all-work" href="work.html">Все работы ${arrowIcon}</a></div><section class="contact-section compact-contact"><div class="shell"><p class="eyebrow">Есть задача?</p><h2>Давайте<br><span>поработаем вместе.</span></h2><a class="contact-circle" href="contact.html">Связаться ${arrowIcon}</a></div></section>`;
