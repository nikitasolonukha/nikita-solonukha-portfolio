...
const workIds = ['legal-automation','internal-legal','shopify-store-builder','meta-ads','protective-structures','magnum','pifpaf','portnoy','support-rag','vibe-autorouter','maps-lead-generator','ritm','photo-animation','garmony'];
const caseDetails = {
...
function casePage(projects) {
  ...
  const feature = (id === 'magnum' ? magnumFeature() : id === 'pifpaf' ? pifpafFeature() : id === 'vibe-autorouter' ? autorouterFeature() : id === 'maps-lead-generator' ? b2bFeature() : projectFeature(id)) ||
    ...
    id === 'support-rag' ? `<section class="case-feature case-feature-split"><div class="shell"><p class="eyebrow">Обращение → оператор → ответ</p><h2>Поддержка в <em>диалоге.</em></h2><div class="case-feature-pair"><figure><img src="${asset('../projects/support-rag/assets/telegram-support-sanitized.png?v=redaction-v2')}" alt="Обезличенный Telegram-топик с обращением и черновиком ответа"><figcaption>01 / Аккаунт поддержки · операторский Telegram</figcaption></figure><figure><img src="${asset('../projects/webchat/assets/conversation-1440.png')}" alt="Локальная демо-версия webchat как второго канала обращений"><figcaption>02 / Webchat · дополнительный вход</figcaption></figure></div><p>После проверки оператором ответ отправляется клиенту от аккаунта поддержки. Сквозная отправка в публичном видео не показана.</p></div></section>` : '');
  ...
  const mainVideo = media.length ? `<section class="case-video-section"><div class="shell"><p class="eyebrow">Продукт в действии</p>${videoFigure(media[0], 0, item.name)}</div></section>` : `<section class="case-archive-section shell"><p>${id === 'ppbot' ? 'Ниже — сохранённые экраны Telegram-бота: меню, фильтры и результат выбора.' : 'Полная оригинальная запись интерфейса пока не найдена. Ниже — сохранившиеся материалы.'}</p></section>`;
